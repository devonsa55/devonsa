...
function generateTargetCoordinates(
  ...
  if (shape === 'chat-chart') {
    ...
    ctx.stroke()
  } else if (shape === 'dual-surfaces') {
    const cardWidth = 68
    const cardHeight = 96
    const gap = 16
    const topY = centerY - cardHeight / 2

    // Card 1 (Left - Surface A: Editorial with smooth curve)
    const card1X = centerX - cardWidth - gap / 2
    ctx.lineWidth = 10
    ctx.strokeStyle = '#000000'
    ctx.strokeRect(card1X, topY, cardWidth, cardHeight)

    // Subtle header line in Card 1
    ctx.fillRect(card1X + 12, topY + 16, cardWidth - 24, 6)

    // Editorial smooth sparkline wave inside Card 1
    ctx.beginPath()
    ctx.moveTo(card1X + 10, topY + 68)
    ctx.bezierCurveTo(
      card1X + 24,
      topY + 40,
      card1X + 44,
    ...
    // Card 2 (Right - Surface B: Constructivist with blocky equalizer bars)
    const card2X = centerX + gap / 2
    ctx.lineWidth = 10
    ctx.strokeRect(card2X, topY, cardWidth, cardHeight)

    // Chunky solid metric box header
    ctx.fillRect(card2X + 10, topY + 14, cardWidth - 20, 12)

    // 3 Equalizer Bars inside Card 2
    const barW = 9
    const barSpacing = 7
    const startX = card2X + 12
    const baseY = topY + cardHeight - 16

    // Bar 1
    ctx.fillRect(startX, baseY - 22, barW, 22)
    // Bar 2
    ctx.fillRect(startX + barW + barSpacing, baseY - 38, barW, 38)
    // Bar 3
    ctx.fillRect(startX + (barW + barSpacing) * 2, baseY - 28, barW, 28)
  }
...
export const DitheredParticles: React.FC<DitheredParticlesProps> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const targetWeight = (
        supportsHoverRef.current ? isHoveredRef.current : isMobileInViewRef.current
      )
        ? 1
        : 0
      ...
      for (let i = 0; i < particlesArray.length; i++) {
        ...
        if (
          hovered &&
          totalDelta < 0.01 &&
          Math.abs(p.targetX - p.x) < 0.1 &&
          Math.abs(p.targetY - p.y) < 0.1
        ) {
          // Sleep stationary
        } else {
          if (hovered) {
            ...
            if (idleShape === 'sphere') {
              ...
              p.vx +=
                (centerX + Math.cos(angle) * Math.sin(p.randomIdleOffset) * radius - p.x) *
                0.01 *
                p.density
              p.vy +=
                (centerY + Math.sin(angle) * Math.cos(p.randomIdleOffset) * radius - p.y) *
                0.01 *
                p.density
              ...
              p.vx +=
                (centerX + Math.cos(angle) * (majorRadius + minorRadius) - p.x) * 0.01 * p.density
              p.vy +=
                (centerY + Math.sin(angle) * (majorRadius + minorRadius) - p.y) * 0.01 * p.density
              ...
              const pRad =
                baseDist * 0.15 + Math.sin(time * 0.01 + p.randomIdleOffset) * (baseDist * 0.05)
              p.vx +=
                (targetCx + Math.cos(p.randomIdleOffset + time * 0.01) * pRad - p.x) *
                0.01 *
                p.density
              p.vy +=
                (targetCy + Math.sin(p.randomIdleOffset + time * 0.01) * pRad - p.y) *
                0.01 *
                p.density
              ...
              p.vy +=
                (centerY +
                  Math.sin(normalizedX * (10 / waveWidth) + time * 0.01) * (h * 0.2) -
                  p.y) *
                0.015 *
                p.density
              ...
              const pts = [
                { x: centerX, y: centerY - 100 - wave },
                { x: centerX - 120 - wave, y: centerY + 100 + wave },
                { x: centerX + 120 + wave, y: centerY + 100 + wave },
              ]
              const edgeIdx = i % 3
              ...
              p.vx +=
                (pts[edgeIdx].x + (pts[nextEdgeIdx].x - pts[edgeIdx].x) * ratio - p.x) *
                0.01 *
                p.density
              p.vy +=
                (pts[edgeIdx].y + (pts[nextEdgeIdx].y - pts[edgeIdx].y) * ratio - p.y) *
                0.01 *
                p.density
              ...
              p.vy +=
                (centerY - 75 + ((time * 0.25 + p.randomIdleOffset * 100) % 150) - p.y) *
                0.015 *
                p.density
              ...
              p.vx +=
                (centerX +
                  (i % gridCols) * spacingX -
                  w / 2 +
                  Math.sin(time * 0.02 + p.randomIdleOffset) * 3 -
                  p.x) *
                0.02 *
                p.density
              p.vy +=
                (centerY +
                  Math.floor(i / gridCols) * spacingY -
                  h / 2 +
                  Math.cos(time * 0.02 + p.randomIdleOffset) * 3 -
                  p.y) *
                0.02 *
                p.density
            }
        ...
        if (particleShape === 'square') {
          ...
          ctx.beginPath()
          ctx.arc(snappedX + pixelSize / 2, snappedY + pixelSize / 2, pixelSize / 2, 0, Math.PI * 2)
          ctx.fill()
        } else if (particleShape === 'rounded') {
          ctx.beginPath()
          ctx.roundRect(snappedX, snappedY, pixelSize, pixelSize, pixelSize / 4)
          ctx.fill()
        }
      }
      time++
